...
bool Assets::EmoteStrategy::Apply(Assets* assets, bool refresh_display_theme) {
    ...
    if (emote_display && emote_display->GetEmoteHandle() != nullptr) {
        ...
        // ponytail: 320_240 layout assumes 320px; shift status/charge icons
        // right on narrower screens so they aren't clipped
        if (display->width() < 320) {
            gfx_coord_t shift = (320 - display->width()) / 2;
            for (const char *name : {"status_icon", "charge_icon", "battery_label"}) {
                if (auto *obj = emote_get_obj_by_name(emote_display->GetEmoteHandle(), name)) {
                    gfx_coord_t x = 0, y = 0;
                    gfx_obj_get_pos(obj, &x, &y);
                    gfx_obj_set_pos(obj, x + shift, y);
                }
            }
        }
    }
