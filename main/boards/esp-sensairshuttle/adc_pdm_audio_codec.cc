...
AdcPdmAudioCodec::~AdcPdmAudioCodec() {
    ...
    // ADC input device kept open; delete releases handles
...
void AdcPdmAudioCodec::EnableInput(bool enable) {
    ...
    // ponytail: ADC continuous driver has internal race with esp_codec_dev_close()
    // on ESP32-C5 — adc_continuous_stop() conflicts with its own callback mutex.
    // Keep device open permanently; just gate reads via input_enabled_.
    if (enable && !adc_opened_) {
